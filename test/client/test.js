...
describe('My Client-Server', function() {
  let server, apiUrl;
  ...
  it('should issue GET /update', function() {
    server.respondWith('GET', `${apiUrl}/update`, JSON.stringify(true));
    let callback = sinon.spy();
    updateStatRecur(callback);
    server.respond();
    sinon.assert.calledWith(callback, true);
  });

  it('should issue GET /login', function() {
    server.respondWith('GET', `${apiUrl}/login`, JSON.stringify(true));
    let callback = sinon.spy();
    login(callback);
    server.respond();
    sinon.assert.calledWith(callback, true);
  ...
  it('should issue GET /disconnect', function() {
    server.respondWith('GET', `${apiUrl}/disconnect`, JSON.stringify(true));
    let callback = sinon.spy();
    disconnect(callback);
    server.respond();
    sinon.assert.calledWith(callback, true);
  ...
  describe('My Client functions', function() {
    let originalValue;
    before(function() {
      originalValue = localStorage.getItem('babble');
    });
    ...
    it('should have empty after click on anonymous button', function() {
      clickOnAnonymousBtn();
      let data = JSON.parse(localStorage.getItem('babble'));
      assert.isEmpty(data.userInfo.name);
      assert.isEmpty(data.userInfo.email);
    }); 
    it('should save details in local storage', function() {
      var name = document.getElementById("name");
      var email = document.getElementById("email");
      name.value = 'Reut Mizrachi';
      email.value = 'mizreut@gmail.com';
      clickOnSaveeBtn();
      let data = JSON.parse(localStorage.getItem('babble'));
      assert.equal(data.userInfo.name, name.value);
      assert.equal(data.userInfo.email, email.value);
    }); 

    it('should add new message', function() {
      var msgList = document.querySelector(".MessagesChat");
      var msg = {
          append : [{
            message: 'Hello',
            name: 'Reut Mizrachi',
            email: 'mizreut@gmail.com',
            time: '13:30',
            md5Image: 'somemd5code',
            msgId:'5'
        }]
      };
      displayMessages(msg);
      assert.equal(msgList.childElementCount, 1);
    }); 
    it('should remove message from chat', function() {
      var msgList = document.querySelector(".MessagesChat");
      removeMessageFromDisplay(5);
      assert.equal(msgList.childElementCount, 0);
    }); 
